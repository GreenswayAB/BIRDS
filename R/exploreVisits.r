...
exploreVisits<-function(x, visitCol=attr(x, "visitCol"), sppCol="scientificName"){
  ...
  datGBY <- dplyr::group_by(dat, !!! rlang::syms(visitCol))

  visitStat$nObs <- dplyr::summarise(datGBY, nObs= dplyr::n())$nObs
  visitStat$SLL  <- dplyr::summarise(datGBY, nSpp= dplyr::n_distinct(scientificName)  )$nSpp
  visitStat$day  <- dplyr::summarise(datGBY, day = dplyr::unique(day))$day
  visitStat$month<- dplyr::summarise(datGBY, mon = dplyr::unique(month))$mon
  visitStat$year <- dplyr::summarise(datGBY, yea = dplyr::unique(year))$yea
  rm(datGBY)
