...
logObsInd<-function(focal,
  ...
  res<-log( res2log + ifelse(res2log==0, .1,0) ) ##+0.368 if it is zero in the denominator it ends up -1
  if (norm){
    res<-normalize(res)
  }
  return(res)
...
obsIndexTemporal<-function(x, timeRes, focalSp=NULL){
  ...
  res$relObs<-logObsInd(res$spN, res$allN)
...
obsIndexSpatial<-function(x, focalSp=NULL){
  ...
  r$relObs <-logObsInd(r$spN, r$allN)
